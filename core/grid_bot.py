...
class AdvancedGridBot:
    ...
    def _initialize_monitoring_session(self):
        """Инициализация сессии мониторинга"""
        ...
        end_time = self._calculate_session_end_time(start_time)
        ...
        last_grid_time = self._initialize_first_grid()
        self._save_state_safe()
        return initial_total_balance, start_time, end_time, last_grid_time, False

    def _calculate_session_end_time(self, start_time):
        """Расчет времени окончания сессии"""
        if hasattr(self, 'session_end_time') and self.session_end_time:
            ...
            print(f"⏰ Ожидаемое время завершения: {completion_time.strftime('%Y-%m-%d %H:%M:%S')}")
        return end_time

    def _initialize_first_grid(self):
        """Инициализация первой сетки ордеров"""
        if not self.trading_paused:
            ...
            self.order_manager.create_grid(self.symbol, grid_config, self.initial_price)
            last_grid_time = time.time()
            self.grid_count = 1
        else:
            print("⏸️ Бот запущен в режиме паузы. Сетка не создана.")
            last_grid_time = 0
        return last_grid_time

    def _process_monitoring_iteration(self, initial_total_balance, end_time, last_grid_time):
        """Обработка одной итерации мониторинга"""
        # Проверяем команды Telegram
        self.telegram_bot.check_commands(self)
        if self.user_commanded_stop:
            print("\n🛑 Получена команда остановки...")
            return True, last_grid_time
        ...
        if self._check_stop_conditions(net_profit, initial_total_balance):
            return True, last_grid_time
        # Периодический отчет и пересоздание сетки
        self._send_periodic_report_if_needed(current_usdt, current_btc, current_price, net_profit)
        last_grid_time = self._recreate_grid_if_needed(last_grid_time, current_price)
        return False, last_grid_time
    ...
    def _calculate_and_log_metrics(self, current_usdt, current_btc, current_price,
        ...
        print(f"\r📊 Активных: {active_orders:2d} | Исполнено: {self.executed_orders_count:2d} | "
              f"Прибыль: {net_profit:+.4f} USDT | Сетка: #{self.grid_count} | "
              f"Ошибки: {self.api_errors} | Осталось: {time_left:.1f} мин", end="")
        return net_profit, time_left
    ...
    def _recreate_grid_if_needed(self, last_grid_time, current_price):
        ...
        if (not self.trading_paused and
            ...
            self._update_commission_rates_if_needed()
            grid_config = {
                'levels': self.grid_levels,
                'order_size': self.order_size,
                'spacing': self.grid_spacing
            }
            self.order_manager.create_grid(self.symbol, grid_config, current_price)
    ...
    def _perform_ai_optimization(self):
        """Выполнение AI оптимизации параметров"""
        if self.ai_mode:
            try:
                ai_recommendations = self.ai_optimizer.get_optimized_parameters(
                    self.price_history, self.monitoring_duration
                )
    ...
    def _update_commission_rates_if_needed(self):
        """Обновление комиссий при необходимости"""
        if self.grid_count % 8 == 0:
            print("🔄 Обновление комиссий...")
            if self.commission_tracker.update_commission_rates():
                rates = self.commission_tracker.get_current_rates()
                print(f"✅ Комиссии обновлены: maker={rates['maker_fee']*100:.4f}%, "
                      f"taker={rates['taker_fee']*100:.4f}%")
    ...
    def run_ai_enhanced_monitoring(self):
        ...
        try:
            ...
            init_result = self._initialize_monitoring_session()
            if init_result[4]:  # stop_triggered
                return 0, True
            initial_total_balance, start_time, end_time, last_grid_time, stop_triggered = init_result
            # Основной цикл мониторинга
            stop_triggered = self._run_monitoring_loop(
                initial_total_balance, end_time, last_grid_time
            )
            # Завершение сессии
            return self._finalize_monitoring_session(initial_total_balance, stop_triggered)
    ...
    def _run_monitoring_loop(self, initial_total_balance, end_time, last_grid_time):
        """Запуск основного цикла мониторинга"""
        stop_triggered = False
        while (time.time() < end_time and not stop_triggered 
               and not self.shutdown_requested):
            try:
                stop_triggered, last_grid_time = self._process_monitoring_iteration(
                    initial_total_balance, end_time, last_grid_time
                )
    ...
    def _finalize_monitoring_session(self, initial_total_balance, stop_triggered):
        ...
        # Очистка состояния
        self._cleanup_session_state(stop_triggered)
    ...
    def _cleanup_session_state(self, stop_triggered):
